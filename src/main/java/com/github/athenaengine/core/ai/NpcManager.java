		...
		switch (st.nextToken())
		{
			...
			case "vote":
				...
				if (checkPlayerCondition(l2PcInstance, null))
				{
					...
					if (container != null)
					{
						VoteManager.getInstance().increaseVote(player, container.getSimpleEventName());
	...
	private static void sendHtmlIndex(Player player)
	{
		...
		else if (EventEngineManager.getInstance().isOpenVote())
		{
			...
			for (IEventContainer container : EventLoader.getInstance().getEnabledEvents())
			{
				...
				StringUtil.append(eventList, "<td width=40%><font color=LEVEL>" + MessageData.getInstance().getMsgByLang(player, "button_votes", false) + ": </font>" + VoteManager.getInstance().getCurrentVotesInEvent(container.getSimpleEventName()) + "</td>");
			...
			html.replace("%eventCountVotes%", " <font color=LEVEL>" + VoteManager.getInstance().getAllCurrentVotesInEvents() + "</font><br1>");
